...
def find_male(director):
    with open("C:\\Mini_Project\\WIKIDATA_DB\\male_directors.tsv", "r", encoding='UTF8') as men_director:
...
def is_male_director(directors):
        for director in directors:
            if (find_male(director)==True):
                return True
        return False

def find_directors(tconst,crew_list):
        for row in crew_list:
                if (row['tconst']==tconst):
                        directors = row['directors']
                        if (directors!="\\N"):
                            directors_list = directors.split(',')
                            result= is_male_director(directors_list)
                            if (result==False):
                                return False
                            else:
                                return True
        ...
        with open("C:\Mini_Project\IMDB_DB\\title.crew.tsv\\title_crew_data.tsv", "r", encoding='UTF8') as crew:
                ...
                for movie in movies_list:
                    ...
                    if (year != '\\N' and int(year) < 2030 and (type == 'short' or type == 'movie')):
                        director=find_directors(movie['tconst'],crew_list)
                        if (director!=False):
                            print(movie['tconst']," ",director)
